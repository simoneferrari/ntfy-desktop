...
- **Preview & apply against history.** `PackHistorySimulator` runs one pack over an ordered history slice (reusing the engine's static `RuleEngine.EvaluateAgainst` + an `InMemoryIncidentStore`) to show what it *would* do, including detected `expect` absence windows. `RulePackHistoryService.Apply` is an **additive backfill**: it re-runs the selected pack and OR-s its hide/fold into the stored `suppressed` flag (via `SuppressMessage`), never clearing it, never touching toasts, skipping `expect`. There is deliberately no auto-undo (a full recompute/reset is deferred).
