...
| Security | DPAPI (`ProtectedData`, `CurrentUser`) for access token + Basic-auth password + history-DB key at rest |
...
- `TopicConnection` refuses to send the `Authorization` header over `ws://` (cleartext). The header value comes from `ServerConfig.GetAuthorizationHeader()` — `Bearer <token>` or `Basic <base64(user:pass)>` depending on the server's `AuthMethod` — the single source of truth for authenticating any request to a server (subscribe socket + attachment download).
...
The notification rule engine (`Features/Rules/`) suppresses noise, folds paired problem/resolved events, and raises heartbeat/absence alerts. Rules live as **declarative JSON packs** under `App.DataPath\rules\*.json` — the format is the API shared by the AI draft dialog, the manager UI, and a future community library. `PackParser` reads packs (tolerating unknown fields — fail open); `RuleEngine.Evaluate` returns a `RuleVerdict` (toast-suppression and feed-hiding are separate axes), with the only persisted axis being the `messages.suppressed` flag. See the design specs for the engine internals.

**Phase 2 — the in-app pack manager** (`Features/Rules/Editor/`) lets users browse/create/edit/enable-disable/delete packs and rules without touching JSON, launched from Settings → Rules ("Manage rule packs…").

- **`enabled` + stable `id` on the pack JSON.** Each pack and rule carries an `enabled` flag (absent ⇒ `true`, so Phase-1 packs are unchanged) and each rule a stable `id` (absent ⇒ the legacy `pack#index`, synthesised on load; a real id is minted when the manager first saves the rule). The id keys `IncidentStore`/`ExpectationStore` state, so editing/reordering no longer orphans it.
- **`PackStore` has two views.** `Packs` (engine-facing) is filtered to *enabled packs, enabled rules only*, so `RuleEngine`/`ExpectationMonitor` stay oblivious to disable state; `GetEditablePacks()` returns everything with file paths for the editor. `Overwrite`/`Delete` mutate a specific file and reload.
- **`PackWriter`** is the inverse of `PackParser` — canonical serialisation (behaviourally-inert fields and comments are not preserved). Editor view-models (`PackViewModel`/`RuleViewModel` subtypes/`MatcherViewModel`) validate before save.
- **Guided authoring (editor VMs).** The forms don't make users write regex. A shared `MatcherViewModel` exposes each title/body field as text + a `MatchMode` (**Contains** / **Equals** / **Regex**); Contains/Equals are `Regex.Escape`-d into the stored pattern, Regex is raw (and how a loaded pack is shown, since an escape can't be reversed). Topic is a dropdown of subscribed topics labelled `Group / DisplayName` but storing the raw ntfy name. `ExpectRuleViewModel` durations are a number + unit (`DurationUnit`) so they're valid by construction. `CorrelateRuleViewModel` keys have a `KeyMode`: **Simple** builds `label\s*(?<key>id)` from a "text before the id" field + an id-shape (`Number`/`Word`/`Anything`), **Regex** is raw. A **"Pick…"** button (`SampleMessagePickerWindow`) fills a field from a recent stored message, and `RuleTemplates` seeds pre-filled starter rules. All VMs `TryValidate` before save (regex compiles, required fields), and `PackViewModel.ToJson` goes through `PackWriter`.
- **Preview & apply against history.** `PackHistorySimulator` runs one pack over an ordered history slice (reusing the engine's static `RuleEngine.EvaluateAgainst` + an `InMemoryIncidentStore`) to show what it *would* do, including detected `expect` absence windows; results surface in a dedicated `RulePreviewWindow` (effect-badged rows + a gaps section). `RulePackHistoryService.Apply` is an **additive backfill**: it re-runs the selected pack and OR-s its hide/fold into the stored `suppressed` flag (via `SuppressMessage`), never clearing it, never touching toasts, skipping `expect`. There is deliberately no auto-undo (a full recompute/reset is deferred). Preview and apply share one **scope** (topic + row-limit, default **All**) so the previewed count is exactly what apply commits. The manager footer separates **Apply** (save, keep editing) from **Save** (save and close).
...
|---|---|
| Credentials at rest | Access token and Basic-auth password DPAPI-encrypted (`CurrentUser` scope) in `settings.json` (username stored plaintext) |
| Credentials over cleartext | `TopicConnection` refuses the `Authorization` header (Bearer or Basic) over `ws://` / `http://` |
| Attachment downloads | `Authorization` header (Bearer or Basic) sent only to a same-origin (scheme+host+port) **https** attachment URL on the topic's server — never to a publisher-supplied external host, never over cleartext (`AttachmentImageService.ResolveAuthHeader`) |
