...
- **Cut the final `-dev.N` *after* the last feature merges, then promote to stable — never skip straight from an older dev tag to the stable tag.** The channels are walled: tagging `v0.7.0` only writes `releases.stable.json`, so dev installs (which read `releases.dev.json`) never receive a feature that merged after the last dev tag. Correct order for a milestone: (1) merge the last feature, (2) bump + tag `v0.7.0-dev.N` (dev channel gets it), (3) bump + tag `v0.7.0` (stable channel gets it). If you realise *after* shipping stable that a feature never got a dev build, recover with the hotfix side-branch pattern below: branch from the stable commit, bump to the next `-dev.N`, tag it (master stays at the stable version — don't walk it backward).
